...
//função de envio
bool VlcTxNetDevice::EnqueueDataPacket(Ptr<Packet> p,bool isACK, bool packetCoded,
										 bool firstPacket, bool lastPktDropped, int retransmissions) {
	...
	if(retransmissions == 0 && lastPktDropped){
		//std::cout << "Incrementou o ID do pacote: --" << lastPktDropped<< "-- " << std::endl;
		m_pktId++;
		//aux = false;
	}
	...
	dropped = m_channel->TransmitDataPacket(p,isACK);
	
	if(!dropped && isACK){
		m_pktId++;
		//aux = true;
	}/*else if(dropped && !packetCoded && aux){
		m_pktId++;
		aux = true;
	}*/
	std::cout << "ID1= " << hdr.GetPacketId1 ()<< " ID2= " << hdr.GetPacketId2 () << " ACK? " << isACK << std::endl;
	std::cout << " Dropou? " << dropped << std::endl;
	if(isACK && !dropped){
