...
void
VlcNetDevice::Receive (Ptr<Packet> p){
	//std::cout << "Recepcao" << std::endl;
}

bool
VlcNetDevice::Send (Ptr<Packet> packet,
                    const Address &dest,
...
Ptr<Packet>
VlcNetDevice::XorPackets(Ptr<Packet> a, Ptr<Packet> b, uint32_t size)
{
    //NS_ASSERT(a->GetSize() == b->GetSize());

    //uint32_t size = a->GetSize();
    std::vector<uint8_t> bufA(size);
    std::vector<uint8_t> bufB(size);

    a->CopyData(bufA.data(), size);
    b->CopyData(bufB.data(), size);
    ...
    return Create<Packet>(bufA.data(), size);
}


void
VlcNetDevice::PrintPacketPayload (Ptr<Packet> p)
{
    ...
    std::cout << "Packet payload (" << size << " bytes): ";
    for (uint32_t i = 0; i < size; ++i)
    {
