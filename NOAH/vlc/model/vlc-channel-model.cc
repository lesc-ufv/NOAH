...
//Precisa retornar um booleano e ir levando ele ate chegar na writebuffer
//assim a write que é a funçao responsável por toda a transmissão e retransmissão
//pode ver se o pacote foi dropado ou não

bool VlcChannel::TransmitDataPacket(Ptr<Packet> p,bool isACK) {
	//NS_LOG_FUNCTION(this<<p);
	...
	int size = p->GetSize();
	//std::cout << "erRate: " << erRate << std::endl;
	
	double packetErrorRate = 1.0 - std::pow((1 - erRate), size*8);

	//std::cout <<"PacketerrorRate calculation: " << packetErrorRate << std::endl;
	...
	//std::cout << "Pacote corrompido? "<< isCorrupt<< std::endl;

	//Verificação aqui se o pacote foi dropado ou não

	rx->EnqueueDataPacketAfterCorruption(p, isCorrupt,isACK);
