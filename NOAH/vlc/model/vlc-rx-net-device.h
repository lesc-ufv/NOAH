...
{
public:
    PacketCircularBuffer(uint32_t capacity);

    bool PushBack(Ptr<Packet> p);     // insere no "fim"
    Ptr<Packet> GetByPacketId(uint32_t id1, uint32_t id2); // procura pacote pelo ID
    bool Remove(uint32_t index);      // remove do meio
    uint32_t Size() const;
...
private:
    ...
    uint32_t m_capacity;
    uint32_t m_head;   // posição do primeiro elemento válido
    uint32_t m_tail;   // próxima posição de escrita
    uint32_t m_size;   // quantos pacotes válidos existem
};
...
public:
	...
	void EnqueueDataPacketAfterCorruption(Ptr<Packet> p, bool, bool isACK);
...
private:
	...
	//tamanho máximo do buffer de pacotes codificados e nao codificados (valor arbitrario)
	PacketCircularBuffer codedBuffer;
	PacketCircularBuffer uncodedBuffer;
