...
static const uint32_t totalTxBytes = 204800;
...
Ptr<Packet> XorPackets(Ptr<Packet> a, Ptr<Packet> b, uint32_t size);

void PrintPacketPayload (Ptr<Packet> p);
...
int main(int argc, char *argv[]) {
	...
	for (double dist = 0; dist < 20; dist += 1) {
		...
		************************************************************************************/
		devHelperVPPM.SetReceiverParameter("THE_RECEIVER1", "SetModulationScheme",VlcErrorModel::OOK);
		...
		*AVILABLE MODULATION SCHEMES [4]. VlcErrorModel::PSK4 [5]. VlcErrorModel::PSK16. [6]. VlcErrorModel::QAM4 [7]. VlcErrorModel::QAM16.
		*************************************************************************************/
		devHelperVPPM.SetReceiverParameter("THE_RECEIVER2", "SetModulationScheme",VlcErrorModel::OOK);
		...
		std::cout << "Distance between TX and RX is " << dist << " meters"
				<< std::endl;
		...
		//double gain = rxHandle->GetRXGain();

		//double error = rxHandle->GetSNRFromErrorModel();*/



		//double calculatedError = rxHandle->CalculateErrorRateForErrorModel();

		/*std::cout <<"Ganho: " << gain << std::endl;
		std::cout <<"Erro: " << error << std::endl;
		std::cout <<"ErroCalculado: " << calculatedError << std::endl;
...
void StartFlow(Ptr<Socket> localSocket, Ipv4Address servAddress,
	...
	localSocket->SetAttribute("SndBufSize", UintegerValue(204800));
...
//mudanças feitas:
/*
Retorno "encadeado" para saber se o pacote foi dropado ou n

ACK será enviado com o toWrite = 0 para representar um pacote sem payload
*/
...
// TODO
/*
 -Ver se esta consistente a quantidade de pacotes que esta sendo enviada 
 Olhando o valores de Bytes restantes, enviados e ate msm a questao do que ainda esta disponivel no codigo

 -Se tiver alguma outra ideia de como validar, tb pode

 -Mas o principal vai ser qnd ta somando os pacotes para ver se ta somando certo
 Se vc comparar com o ARQ, aq tenta duas vezes com um pacote para ver se vai desistir dele ou nn

 Qnd desiste do pacote, ou seja, ultrapassa a quantidade de retransmissões, soma 2048 para representar q um pacote foi pulado
 e soma qnd um pacote foi aceito corretamente, ou seja, qnd um ACK chegou
...
void WriteUntilBufferFull(Ptr<Socket> localSocket, uint32_t txSpace) {
	...
	bool lastPktDropped = false;
	bool packetCoded = false;
	bool firstPacket = true;
	int reset = false;
	Ptr<Packet> previousPacket = Create<Packet>();
	Ptr<Packet> codedPacket = Create<Packet>();
	int qntSentPkts = 0;


	while ((currentTxBytes < totalTxBytes && localSocket->GetTxAvailable() > 0) || isACK) {
		//std::cout << "Comeco do while" << std::endl;
		...
		if(isACK){
			...
			toWrite = 0;
			if(retransmissions > 0 && lastPktDropped){
				//std::cout << " Retransmits atual: " << retransmissions << std::endl;
				isACK = false;
				if(retransmissions > 1 ){
					if(reset){
						currentTxBytes += 2048;
						reset = false;
						std::cout << "Limite de resets atingido, pulando pacote" <<std::endl;
					}else{
						reset = true;
						std::cout << "Resetou uma vez" <<std::endl;
					}
					retransmissions = 0;
					lastPktDropped = true;
		...
		toWrite = std::min (toWrite, left);
		//cria o pacote e salva no buffer 
		...
		//codifica ou nao o pacote
		if(!isACK){
			if(firstPacket){//uncoded packet
				previousPacket = p;
			}//coded packet
			else if(packetCoded){
				codedPacket = VlcNetDevice::XorPackets(p, previousPacket,toWrite);
		...
		if(packetCoded && !isACK){
			p = codedPacket;
		}
		dropped = txOne->EnqueueDataPacket(p,isACK,packetCoded,firstPacket,lastPktDropped, retransmissions);
		...
		if(!dropped){	
			//std::cout << " Pacote atual nao dropou, last = " << retransmissions << std::endl;
			...
			if(isACK){
				std::cout << "Nao era ACK, somou correto" << std::endl;
				int amountSent = localSocket->Send (&data[dataOffset], 2048, 0);
			...
			retransmissions++;
			lastPktDropped = true;
			std::cout << " Pacote atual dropou, retransmissions = " << retransmissions << std::endl;
	...
	//only for print just one
	if(qntSentPkts != 0){
		std::cout << " Quantidade de pacotes enviados: " << qntSentPkts << std::endl;
