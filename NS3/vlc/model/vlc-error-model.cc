...
double VlcErrorModel::CalculateSER(void) {
	// BER = Q(sqrt(SNR)) = 0.5 * erfc( sqrt(SNR) / sqrt(2) )

	//std::cout << "SNR: " << SNR << std::endl;
    double SER = 0.5 * erfc( std::sqrt(SNR) / std::sqrt(2) );
...
double VlcErrorModel::CalculateBER(void)
{
    double snr = SNR;
    if (snr <= 0) {
        return 0.5;   // meia chance
    }

    // OOK BER (Binary Intensity Modulation)
    // BER = Q( sqrt(SNR) )
    double Q = 0.5 * erfc( std::sqrt(snr) / std::sqrt(2.0) );
...
bool VlcErrorModel::IsCorrupt(Ptr<Packet> p) {
	...
	if (mod_scheme == PAM4 || mod_scheme == OOK || mod_scheme == PAM8 || mod_scheme == PAM16) {
		...
		OLHAR ESTA PARTE E ESSA FUNÇÃO EM ESPECIFICO	
		...
		*/
		...
		//std::cout << "PER" <<per <<"Get Rate"<< GetRate() << std::endl;


		double per = GetRate(); // Erro relacionado ao SER
		double x = m_ranvar->GetValue();
		// //std::cout << "RANDOM=" << x << " PER=" << per << std::endl;
		return (x < per);
...
bool VlcErrorModel::CorruptPacket(Ptr<Packet> p, double ber) {
	VlcErrorModel::SetRate(ber);
	bool val = VlcErrorModel::IsCorrupt(p);
	return val;
}
