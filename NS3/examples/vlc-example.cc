...
void WriteUntilBufferFull(Ptr<Socket> localSocket, uint32_t txSpace) {
	...
	while ((currentTxBytes < totalTxBytes && localSocket->GetTxAvailable() > 0) || isACK) {
		...
		if(isACK){
			...
			toWrite = 0;
			//isACK = false;
			if(lastPacketDropped > 0){
				isACK = false;
				if(lastPacketDropped == 2 ){
					lastPacketDropped = 0;
				}
				continue;
		...
		dropped = txOne->EnqueueDataPacket(p,isACK,packetCoded,firstPacket);
	

	
		if(!dropped){	
			//std::cout << " Pacote atual nao dropou, last = " << lastPacketDropped << std::endl;
			...
			//std::cout << " Pacote atual dropou, last = " << lastPacketDropped << std::endl;
			lastPacketDropped++;
		}
